...
class UFormer(nn.Module):
    ...
    def forward(self, data_batch, mask=None):
        inp_img = data_batch['LQ_image']
        gt_img = data_batch['HQ_image']

        if inp_img.shape[2] != self.reso or inp_img.shape[3] != self.reso:
            inp_img = F.interpolate(inp_img, (self.reso, self.reso))

        # Input Projection
        y = self.input_proj(inp_img)
        y = self.pos_drop(y)
        # Encoder
        ...
        # Output Projection
        y = self.output_proj(deconv3) + inp_img

        if y.shape[2] != gt_img.shape[2] or y.shape[3] != gt_img.shape[3]:
            y = F.interpolate(y, (gt_img.shape[2], gt_img.shape[3]))

        loss = self._calculate_loss(y, gt_img)
